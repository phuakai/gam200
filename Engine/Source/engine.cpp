...
void engineInit()
{
	...
	bg.Add<BaseInfo>("background", vector2D::vec2D(0.f, 0.f), vector2D::vec2D(Graphics::Input::screenwidth, Graphics::Input::screenheight));
	...
	for (std::list<fow::fowTile>::iterator it = fow::fowMap.getFowTileMap().begin(); it != fow::fowMap.getFowTileMap().end(); ++it, ++inc)
	{
		...
		cloud[inc].Add<Texture>(3, 1, 1, "Cloud");
	...
	enemyManagerEntity.Add<Texture>(4, 1, 1, "Enemy");
