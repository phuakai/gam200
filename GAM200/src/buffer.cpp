...
void Graphics::VAO::enableattrib(GLuint vao)
{
	glEnableVertexArrayAttrib(vao, 0);// Enables the vertex array attrib for index 0 of vao
...
void Graphics::VAO::bindattrib(GLuint vao)
{
	glVertexArrayAttribBinding(vao, 0, 0); // Associates a vertex attribute and a vertex buffer binding for a VAO
	// Name of vertex array object, index of vertex attrib, index of vertex buffer binding index
}

void Graphics::VAO::setattrib(GLuint vao)
{
	glVertexArrayAttribFormat(vao, 0, 2, GL_FLOAT, GL_FALSE, 0);// Specify the organisation of vertex arrays
...
void Graphics::VBO::bind(GLuint vao, GLuint vbo, GLsizei step)
{
	glVertexArrayVertexBuffer(vao, 0, vbo, 0, step); // Binds a buffer to a vertex array object
...
void Graphics::VBO::setdata(GLuint vbo, int size, std::vector <vector2D::vec2D> data)
{
